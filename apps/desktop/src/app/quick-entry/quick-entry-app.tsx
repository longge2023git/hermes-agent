    ...
    >
      ...
      >
        <div style={{ alignItems: 'center', display: 'flex', gap: 10 }}>
          ...
          <input
            aria-label={t.quickEntryApp?.inputLabel ?? 'Quick Entry'}
            autoCapitalize="off"
            autoComplete="off"
            autoCorrect="off"
        ...
        <div style={{ alignItems: 'center', display: 'flex', gap: 8 }}>
          ...
          >
            {t.quickEntryApp?.sendTo ?? 'Send to'}
          </label>
          <select
            aria-label={t.quickEntryApp?.targetLabel ?? 'Target session'}
            disabled={!state.connected}
            ...
            value={state.target}
          >
            <option value={QUICK_TARGET_CURRENT}>{t.quickEntryApp?.targetCurrent ?? 'Current chat'}</option>
            <option value={QUICK_TARGET_NEW}>{t.quickEntryApp?.targetNew ?? 'New session'}</option>
